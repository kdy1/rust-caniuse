...
", var=var_name,id=key,title=feat.title,parent=feat.parent,status=feat.status).unwrap();
    }


    // print id:Feature map
    f.write_all(b"\n\n").unwrap();
    write!(&mut f, "static FEATURES: phf::Map<&'static str, Feature> = ").unwrap();
...
fn get_data(client: &Client, head: &str) -> Data {
    let u = format!(
        "https://raw.githubusercontent.com/Fyrd/caniuse/{}/data.json", head);
...
fn get_git_head(client: &Client) -> String {
    ...
    let r = get_json::<NpmResponse>(&client,
         &format!("git_head_{}", ver), &u);
...
fn get_json<T: Deserialize>(client: &Client,
    cache_file_name: &str, url: &str) -> T {
    ...
    if let Ok(f) = File::open(&dest_path) {
        return serde_json::from_reader::<_, T>(f)
    ...
    let mut f = File::create(dest_path).expect("create cache");
    f.write_all(&body.into_bytes()).expect("write cache");
